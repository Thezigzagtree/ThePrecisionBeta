...
public class Object_RoamerSlerp : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		for (int i = 0; i < targets.Length; i++) {
			targets [i].transform.position = Vector3.Slerp (targets [i].transform.position, destinations [i], Time.deltaTime * FindObjectOfType<itemBank> ().getMoveSpeed () * speed);
		}
